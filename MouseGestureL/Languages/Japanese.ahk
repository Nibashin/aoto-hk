...
ActionName043 = 同クラスのウィンドウを全て元に戻す
ActionName044 = 同クラスのウィンドウを等間隔に並べる
...
ActionName092 = ジェスチャーの入力状態を記憶1
ActionName093 = ジェスチャーの入力状態を記憶2
ActionName094 = アクティブウィンドウをターゲットにする
...
ME_LngCapt028 = 同クラスのウィンドウを全て元に戻す
ME_LngCapt029 = 同クラスのウィンドウを等間隔に並べる
...
ME_LngText451 = MouseGestureLを再起動:
ME_LngText452 = ジェスチャー有効/無効の切り替え:
ME_LngText453 = ジェスチャーの入力を中止:
ME_LngText454 = ジェスチャーリストを表示:
ME_LngText455 = ナビON/OFFの切り替え:
ME_LngText456 = 高さの制限 ( 0 で無制限 ):
ME_LngText501 = 横幅:
ME_LngText502 = 高さ:
...
ME_LngButton005 = 選択中のターゲットにジェスチャーを追加
ME_LngButton006 = 選択中のジェスチャーにアクションを適用
ME_LngButton007 = 追加
ME_LngButton008 = 更新
ME_LngButton009 = 変更
...
ME_LngCheckBox020 = トリガーボタンを常時フック処理する
ME_LngCheckBox021 = デフォルトジェスチャーを適用しない
ME_LngCheckBox022 = 同方向の長距離ストロークを連続認識しない
...
ME_LngCheckBox114 = 「同クラスのウィンドウを全て最小化」 を使って最小化したウィンドウを元に戻す
...
ME_LngMessage120 = このアクションの利用方法は###LinkStart###Webサイトの解説###LinkEnd###を参照してください。`n
...
[RB_]
G=RB_
Default=;アクティブ化<MG_CR>if (InStr(MG_WClass, "Chrome_WidgetWin")!=1 || (MG_WStyle & 0x000B0000)) {<MG_CR><MG_TAB>MG_WinActivate()<MG_CR>}
