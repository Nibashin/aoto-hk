...
MG_InitCommonGlobals()
{
	...
	MG_HIDEHINTS	 := "<HideHints>"
	MG_COPYRIGHT1	 := "Copyright (C) 2007-2008 lukewarm"
	MG_COPYRIGHT2	 := "Copyright (C) 2011-2025 Pyonkichi"
	MG_WEBSITEURL	 := "https://ss1.xrea.com/pyonkichi.g1.xrea.com/"
	MG_HOMEPAGE		 := "mglahk.html"
	MG_TIPSPAGE		 := "mglahk_tips.html"
	MG_SUPPORTURL	 := "https://www.autohotkey.com/boards/viewtopic.php?f=6&t=31859"
...
MG_Edit()
{
	...
	MG_SearchPlugins()
	t1 := t2 := 0
	szConfAhk := MG_DirConfig "MG_Config.ahk"
	FileGetTime, t1, %szConfAhk%
	RunWait, %A_AhkPath% "%A_ScriptDir%\MG_Edit.ahk"
	if (FileExist(szConfAhk)) {
		FileGetTime, t2, %szConfAhk%
		if (t2 > t1) {
			Reload
...
MG_CheckLanguage(fChoose=0)
{
	...
	static	tblLang := [ [ "Japanese.ahk", "0411" ]
					   , [ "Chinese.ahk" , "0804", "0C04", "1004", "0404", "1404" ] ]
...
; Search Plugins
;	Return Value
;	 True  : Plugin Include Script has been Updated
;	 False : Plugin Include Script has Not been Changed
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_SearchPlugins()
{
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_DrawGesture(hDC, ptX, ptY, szGesture, ByRef strW=0, ByRef strH=0, fMeasure=0, iCurLen=0)
{
	global MG_BtnNames, MG_AdNaviFG2, MG_AdNaviNI2, MG_hFntBtn, MG_hFntDir, MG_AdNaviSize
	...
	hPen := hPen2 := hPenOld := 0
	if (!fMeasure) {
		hPen := DllCall("CreatePen", Int,0, Int,1, Int,MG_AdNaviFG2)
		hPenOld := DllCall("SelectObject", Ptr,hDC, Ptr,hPen, Ptr)
	}
	hFntOld := DllCall("SelectObject", Ptr,hDC, Ptr,MG_hFntBtn, Ptr)
	VarSetCapacity(size, 8, 0)
	DllCall("GetTextExtentPoint32", Ptr,hDC, Str,"_", Int,1, Ptr,&size)
	btnupW	:= NumGet(size, 0, "UInt")
	btnupY	:= NumGet(size, 4, "UInt") // 6
	charH	:= MG_AdjustToDPI(MG_AdNaviSize)
	nSteps	:= StrLen(szGesture)
	preFont	:= 1
	nowX	:= ptX
	maxH	:= 0
	pos		:= 1
	while (pos <= nSteps) {
		...
		if (SubStr(szGesture, pos, 1) == "_") {
			...
			if (!fDown) {
				...
				offset := 1
				shift := charH//4
				shift -= (dir="DirU" || dir="Dir8") ? charH//6 : 0
				shift += (dir="DirD" || dir="Dir2") ? charH//6 : 0
			}
		}
		if (preFont != newFont) {
			preFont := newFont
		...
		if (!fMeasure) {
			if (szDraw=="_" && !fDown) {
				DllCall("MoveToEx", Ptr,hDC, Int,nowX, Int,ptY+btnupY, Ptr,0)
				DllCall("LineTo", Ptr,hDC, Int,nowX+btnupW, Int,ptY+btnupY)
			} else {
			...
			if (pos == iCurLen+1) {
				...
				hPen2 := DllCall("CreatePen", Int,0, Int,1, Int,MG_AdNaviNI2)
				DllCall("SelectObject", Ptr,hDC, Ptr,hPen2, Ptr)
			}
		}
		if (szDraw == "_") {
			fDown := 0
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_CreateMutexObject(name) {
	return DllCall("CreateMutex", Ptr,0, UInt,0, Str,name, Ptr)
}
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_ReleaseMutexObject(hMutexObj) {
	if (hMutexObj) {
		DllCall("CloseHandle", Ptr,hMutexObj)
	}
}
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_GetMutex(name, timeout) {
	hMutex := DllCall("OpenMutex", UInt,0x001F0001, UInt,0, Str,name, Ptr)
	if (hMutex) {
		result := DllCall("WaitForSingleObject", Ptr,hMutex, UInt,timeout, UInt)
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_GetExeName(hWnd:=0, bPath:=false)
{
	local szExe, hChild
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_IsActivationTarget(hWnd, bIncHidden:=false, bExcSpecified:=true)
{
	local dwStyle, dwExStyle, szTitle, szClass

	WinGet, dwStyle, Style, ahk_id %hWnd%
	WinGet, dwExStyle, ExStyle, ahk_id %hWnd%
	if ((!bIncHidden && !(dwStyle&0x10000000))					; not WS_VISIBLE
	||	(dwStyle&0x08000000) || (dwExStyle&0x00000080)			; or WS_DISABLED or WS_EX_TOOLWINDOW
	||	((dwStyle&0x80000000) && !(dwStyle&0x20C80000))) {		; or (WS_POPUP and not WS_MINIMIZE and not WS_CAPTION and not WS_SYSMENU)
		return false
	}
