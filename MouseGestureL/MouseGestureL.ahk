...
MG_CheckLanguage()
if (MG_SearchPlugins() && MG_CheckConfigFiles()) {
	Reload
}
MG_CheckConfigFiles()
MG_RegisterMenu()
...
MG_RegisterMenu()
{
	...
	Menu, %MG_MenuName%, Add, %MG_LngMenu003%, MG_ToggleHints
...
MG_InitGlobals()
{
	...
	; Mutex Objects
	MG_MUTEX_ACTIVATE	:= "MOUSEGESTUREL_MUTEX_ACTIVATE"
	MG_MUTEX_SAVEACTWND	:= "MOUSEGESTUREL_MUTEX_SAVEACTWND"
	MG_MUTEX_DELACTWND	:= "MOUSEGESTUREL_MUTEX_DELACTWND"
	MG_MUTEX_HINTSARW	:= "MOUSEGESTUREL_MUTEX_HINTSARW"
	MG_MUTEX_HINTSADV	:= "MOUSEGESTUREL_MUTEX_HINTSADV"
	MG_hMutex := []
	MG_hMutex.Push(MG_CreateMutexObject(MG_MUTEX_ACTIVATE))

	; Dummy Window
	Gui, MGW_Main:New
	Gui, MGW_Main:+HwndMG_hwndMain -Caption +ToolWindow
	Gui, MGW_Main:Show, x0 y0 w0 h0 Hide, MouseGestureL.ahk Main Window
	OnMessage(0x0010, "MG_OnClose")
}
...
;-------------------------------------------------------------------------------
MG_OnClose(wParam, lParam, uMsg, hWnd) {
	MG_Exit()
}
...
MG_ShowAboutDlg()
{
	...
	MG_AboutLinks := [["AboutWebIcon"	, "S14 w1000" , "Webdings"	, 2]
					...
					, ["AboutConIcon"	, "S12"		  , "Wingdings"	, 4]
					, ["AboutContact"	, "Underline" , ""			, 3]]
	MG_LinkHover := [ 0, 0, 0, 0 ]
	...
	Gui, MGW_About:Add, Text, xs+8 y+10, %MG_COPYRIGHT1%
	Gui, MGW_About:Add, Text, xs+8 y+4,  %MG_COPYRIGHT2%
...
OnLinkClick:
	if (A_GuiControl="AboutWebIcon" || A_GuiControl="AboutWebsite") {
		Run, % MG_WEBSITEURL . (MG_Language="Japanese" ? "" : "en/") . MG_HOMEPAGE
	}
...
MG_GetMousePosInfo()
{
	...
	SendMessage, 0x84, 0, % (MG_Y & 0xFFFF)<<16|(MG_X & 0xFFFF),,ahk_id %MG_HCTL%
	...
	IfWinExist,ahk_id %MG_HWND%,,,,{
		...
		WinGet, MG_PID, PID
		WinGet, MG_WStyle, Style
		WinGet, MG_ExWStyle, ExStyle
...
MG_CorrectDlgCtrlHandle()
{
	...
	hParent := DllCall("GetParent", Ptr,MG_HCTL, Ptr)
	hCtrl := MG_HCTL
	while (hCtrl)
	{
...
MG_TriggerDown(name)
{
	...
	if (!InStr(MG_CurTriggers, name . "_")) {
		...
		if (MG_Active && MG_TimedOut) {
			...
			; それ以外の場合、ジェスチャーの発動判定
			MG_Recognition()
			if (MG_BypassGesture && InStr("_" MG_BypassTrigger "_", "_" name "_")) {
				MG_Gesture := MG_BypassGesture . name "_"
			} else {
				MG_Gesture .= name "_"
			}
			MG_Check()
...
MG_TriggerUp(name)
{
	...
	if (!MG_Active || MG_TimedOut) {
		...
		if (MG_TriggerCount < 1) {
			if (MG_NaviPrstStr) {
				MG_NaviPrst := 1
				SetTimer, MG_HintsPersistTimer, %MG_NaviPersist%
			}
			MG_StopNavi(0)
			MG_StopTrail()
			if (MG_CancelDef) {
...
MG_Check(g="")
{
	...
	if (StrLen(MG_Gesture) > MG_MaxLength) {
		...
		if (IsLabel("MG_Gesture_" . ges)) {
			...
			if ((MG_HWND != MGW_hwHintsArw) && (MG_HWND != MG_TrailHwnd)) {
...
{
	global
	static px, py, plx, ply, dir, prev_dir, prev_long, orange
	local powx, powy
	;...........................................................................
	; Initialization of recognition process
	if (fInit) {
		MG_Active:=1, MG_Aborted:=0, MG_WaitNext:=0, MG_Executed:=0, MG_TimedOut:=0
		px:=plx:=MG_X, py:=ply:=MG_Y, dir:="", prev_dir:="", prev_long:=""
		orange:=MG_ORange%MG_ORangeDefault%, MG_PrevGesture:="",
	...
	if (MG_TriggerCount < 1) {
		...
		MG_Active:=0, MG_TriggerCount:=0, MG_CurTriggers:="", MG_BypassTrigger:="", MG_BypassGesture:=""
		Loop, Parse, MG_SubTriggers, _
		{
			GoSub, MG_%A_LoopField%_Disable
		}
	...
	&&		 (MG_Aborted
			  || (MG_TimeoutEnabled && MG_Timeout && (A_TickCount>MG_TimeLimit) && !MG_Executed)))
	{
		...
		if (dir != prev_dir) {
			...
			if (fChanged) {
				if (MG_FirstLongOnly && prev_long==dir) {
					fChanged := 0
				}
				prev_long := dir
			}
...
;-------------------------------------------------------------------------------
; Assume the action has not executed and continue the current gesture
;														Implemented by lukewarm
;-------------------------------------------------------------------------------
MG_Cancel() {
	MG_Wait(0)
...
MG_SetTimeout() {
	global
	MG_TimeLimit := A_TickCount + MG_Timeout
	MG_TimeoutEnabled := true
}
...
MG_SaveGesture() {
	global
	MG_Snapshot := MG_Gesture
	MG_Cancel()
}

;-------------------------------------------------------------------------------
; Strokes following this function call will be ignored if the specified
; sub-trigger is pressed.
;	btn	: Button name referenced as the sub-trigger.
;		  If it is omitted all subtriggers are referenced.
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_SetBypass(btn:="") {
	global
	MG_BypassTrigger := (btn!="") ? btn : MG_SubTriggers
	MG_BypassGesture := MG_Gesture
}
...
MG_HitTest(c=0)
{
	...
	if (c) {
		...
		SendMessage,0x84,0,% (MG_Y & 0xFFFF)<<16|(MG_X & 0xFFFF),,ahk_id %MG_HWND%
	}
	return (ErrorLevel<8)
			? ((ErrorLevel<4)
				? ((ErrorLevel<2)
...
MG_Scroll2(dir, cnt, fPage=0)
{
	...
	Loop, 2
	{
		...
		if (MG_hSB) {
			lParam := MG_hSB
			hWnd := DllCall("GetParent", Ptr,MG_hSB, Ptr)
			break
		}
		if (A_Index==1) {
			hWnd := DllCall("GetParent", Ptr,MG_HCTL, Ptr)
		}
	}
	if (!lParam) {
		hWnd := MG_HCTL
...
MG_SendWheel(dir, counts=1)
{
	...
	lParam := (MG_Y & 0xFFFF)<<16 | (MG_X & 0xFFFF)
...
;-------------------------------------------------------------------------------
; Move the window to a specified location
...
;	bFull	: Adjust window position to the entire area of the monitor
;	hWnd	: Handle of the target window
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_WinMoveSpecific(pos, mon:=1, bAdjust:=0, ofsX:=0, ofsY:=0, bFull:=0, hWnd:=0)
{
	global MG_HWND
	static prvX, prvY, hLastWnd:=0

	area := bFull ? "Monitor" : "MonitorWorkArea"
	SysGet, rc, %area%, % mon!=1 ? mon : ""
	if (rcLeft == "") {
		SysGet, rc, %area%
		if (rcLeft == "") {
			return
	...
	Loop, 2  {
		...
		if (pos = "C") {		; Center
			dstX := (rcLeft - bdL + ofsX) + (rcRight - rcLeft - w)/2
			dstY := (rcTop - bdT + ofsY) + (rcBottom - rcTop - h)/2
		}
		else if (pos = "L") {	; Left
			dstX := rcLeft - bdL + ofsX
			dstY := orgY + ofsY
		}
		else if (pos = "T") {	; Top
			dstX := orgX + ofsX
			...
			dstX := rcRight - w + bdR + ofsX
			dstY := orgY + ofsY
		}
		else if (pos = "B") {	; Bottom
			dstX := orgX + ofsX
			dstY := rcBottom - h + bdB + ofsY
		}
...
MG_SetWinEventHook()
{
	...
	MG_hwQueue := []
	MG_bQueueUse := false
	MG_hMutex.Push(MG_CreateMutexObject(MG_MUTEX_SAVEACTWND))
	MG_hMutex.Push(MG_CreateMutexObject(MG_MUTEX_DELACTWND))
...
{
	if (iEvent==0x0003 && idObject==0 && idChild==0) {
		MG_StoreActiveWindowInfo(hWnd)
	}
}
...
MG_StoreActiveWindowInfo(hWnd)
{
	local hMutex, idx, max

	hMutex := MG_GetMutex(MG_MUTEX_SAVEACTWND, 0)
	if (!hMutex) {
		MG_PushActWinQueue(hWnd)
		SetTimer, MG_RetryStore, 50
		return
	}
	if (MG_IsActivationTarget(hWnd, true)) {
		idx := max := MG_aryActWnd.MaxIndex()
		DetectHiddenWindows, On
		Loop, %max% {
			if (!WinExist("ahk_id " MG_aryActWnd[idx])
			||	(MG_aryActWnd[idx] == hWnd)) {
...
{
	global
	if (MG_hwQueue.MaxIndex() > 0) {
		MG_StoreActiveWindowInfo(MG_PopActWinQueue())
	}
	if (MG_hwQueue.MaxIndex() <= 0) {
		SetTimer, MG_RetryStore, Off
	}
}
...
{
	global
	MG_tmClrActBuf := (tmLimit<=0) ? 1 : tmLimit
	MG_bActMinWnd := bMin
	if (tmLimit <= 0) {
		MG_DeleteActivationBuffer()
	}
	SetTimer, MG_WinActivationThread, -1
}

MG_WinActivationThread()
{
	local hMutex, hwAct, hwPrev, dwStyle, bMin, hTApp, bActivate

	hMutex := MG_GetMutex(MG_MUTEX_ACTIVATE, 0)
	if (!hMutex) {
		return
	}
	if (MG_idxActWnd <= 0) {
		if (MG_aryActWnd.MaxIndex() < 2) {
			MG_ReleaseMutex(hMutex)
			return
		}
		MG_aryActWndCopy := MG_aryActWnd.Clone()
		MG_idxActWnd := MG_aryActWndCopy.MaxIndex()
	}
	DetectHiddenWindows, On
	WinGet, hwAct, ID, A
	Loop, % MG_aryActWndCopy.MaxIndex() {
		hwPrev := MG_aryActWndCopy[MG_idxActWnd]
		if (WinExist("ahk_id " hwPrev)) {
			WinGet, dwStyle, Style, % "ahk_id " hwPrev
			bMin := (dwStyle & 0x20000000) ? true : false
			hTApp := MG_IsDelphiMinWnd(hwPrev)
			if (MG_bActMinWnd) {
				bActivate := (hwAct!=hwPrev || bMin) ? true : false
			} else {
				bActivate := (hwAct!=hwPrev && !bMin && !hTApp) ? true : false
			}
			if (bActivate) {
				if (hTApp || MG_IsActivationTarget(hwPrev)) {
					WinActivate, % "ahk_id " hwPrev
					if (hTApp) {
						; Restore minimized Delphi application window
	...
	DetectHiddenWindows, Off
	SetTimer, MG_DeleteActivationBuffer, -%MG_tmClrActBuf%
	MG_ReleaseMutex(hMutex)
}
...
MG_DeleteActivationBuffer()
{
	...
	hMutex := MG_GetMutex(MG_MUTEX_DELACTWND, 0)
	if (!hMutex) {
		return
	}
...
MG_Activation()
{
	local hMutex, FocusClassNN, hFocusCtrl

	hMutex := MG_GetMutex(MG_MUTEX_ACTIVATE, 0)
	if (!hMutex) {
		return
	}
	if (!WinExist("ahk_id " MG_HWND) || MG_idxActWnd>0) {
		MG_ReleaseMutex(hMutex)
		return
	}
	if (!MG_bWndActivated) {
...
;
;	szOpe	  : "Minimize", "Close", "Restore"
;				"RestoreMinWnd" = Restores the windows that were previously
;								  minimized by this function.
;	szInTitle : Limits to windows that meet this window title.
;	szExTitle : Excludes windows that meet this window title.
;	szClass	  : Specifies the window class to operate. If omitted, the gesture
;				target class will be the operation target.
;
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_OperateSameClass(szOpe, szInTitle:="", szExTitle:="", szClass:="")
{
	global
	MGOSC_szOpe		:= szOpe
	MGOSC_szInTitle	:= szInTitle
	MGOSC_szExTitle	:= szExTitle
	MGOSC_szClass	:= szClass ? szClass : MG_WClass
	SetTimer, MG_OperateSameClassThread, -1
}

MG_OperateSameClassThread()
{
	local max, hWnd, uMsg, wParam

	if (MGOSC_szOpe = "Minimize") {
		uMsg:=0x0112, wParam:=0xF020
		MGOSC_hMinWnd := []
	} else if (MGOSC_szOpe = "Close") {
		uMsg:=0x0010, wParam:=0
	} else if (MGOSC_szOpe = "Restore" || MGOSC_szOpe = "RestoreMinWnd") {
		uMsg:=0x0112, wParam:=0xF120
	} else {
		return
	}
	if (MGOSC_szOpe = "RestoreMinWnd") {
		max := MGOSC_hMinWnd.MaxIndex()
		Loop, %max% {
		    hWnd := MGOSC_hMinWnd.Pop()
		    if (MG_IsOperationTarget(hWnd, MGOSC_szInTitle, MGOSC_szExTitle)) {
		...
		WinGet, WndList, list, ahk_class %MGOSC_szClass%
		Loop, %WndList%
		{
...
MG_IsOperationTarget(hWnd, szInTitle, szExTitle)
{
	if (szInTitle || szExTitle) {
		WinGetTitle, title, ahk_id %hWnd%
		if ((szInTitle && !InStr(title, szInTitle))
		||	(szExTitle &&  InStr(title, szExTitle))) {
			return false
		}
	}
	return true
...
MG_InitHints()
{
	...
	if (MG_UseNavi) {
		...
		MG_CreateHintsAdvanced()
		if (MG_UseExNavi==1 || MG_UseExNavi==2) {
			MG_CreateHintsArrows()
		}
	}
	MG_hMutex.Push(MG_CreateMutexObject(MG_MUTEX_HINTSARW))
	MG_hMutex.Push(MG_CreateMutexObject(MG_MUTEX_HINTSADV))
}
...
MG_StartNavi()
{
	global
	if (MG_NaviEnabled) {
		SetTimer, MG_HintsPersistTimer, Off
		MG_NaviPrst := 0
		MG_bListShowing := false
		MG_SetArrowHintsBgColor()
		MG_GetTargetMonitorRect()
		MG_UpdateHints()
		SetTimer, MG_UpdateHints, %MG_NaviInterval%
	}
}
...
MG_StopNavi(fForce=1)
{
	...
	if (MG_NaviEnabled) {
		...
		SetTimer, MG_UpdateHints, Off
		Gui, MGW_HintsAdv:Show, Hide
		if (MG_UseExNavi) {
			Gui, MGW_HintsArw:Show, Hide
		} else {
			Tooltip
			MG_Tooltip=
		}
...
MG_ShowGestureList()
{
	global
	if (!MG_bListShowing) {
		MG_bListShowing := true
		MG_DisableTimeout()
		if (MG_UseExNavi != 4) {
			MG_UpdateHintsAdvanced(1)
		}
	} else {
		MG_HideGestureList()
	}
...
MG_HideGestureList()
{
	...
	if (MG_bListShowing) {
		MG_bListShowing := false
		if (MG_UseExNavi < 3) {
			Gui, MGW_HintsAdv:Hide
		} else if (MG_UseExNavi == 3) {
			MG_UpdateHintsAdvanced(2)
		}
		MG_SetTimeout()
	}
}
...
MG_UpdateHints()
{
	...
	if (!MG_bListShowing) {
		if (MG_UseExNavi==1 || MG_UseExNavi==2) {
			MG_UpdateHintsArrows()
		}
		else if (MG_UseExNavi==3 || MG_UseExNavi==4) {
			MG_UpdateHintsAdvanced()
		}
		else {
			MG_UpdateHintsTooltip()
		}
	}
}

;-------------------------------------------------------------------------------
; Update Tooltip Gesture Hints
;														Implemented by lukewarm
;														Modified by Pyonkichi
;-------------------------------------------------------------------------------
MG_UpdateHintsTooltip()
...
;-------------------------------------------------------------------------------
; Create Advanced Gesture Hints Window
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_CreateHintsAdvanced()
{
	...
	Gui, MGW_HintsAdv:New
	Gui, MGW_HintsAdv:+HwndMGW_hwHintsAdv -Caption +ToolWindow +AlwaysOnTop +LastFound -DPIScale
	...
	Gui, MGW_HintsAdv:Color, %MG_AdNaviBG%
	colorBG := MG_AdNaviTransClr
...
{
	global
	Gui, MGW_HintsArw:New
	Gui, MGW_HintsArw:+HwndMGW_hwHintsArw -Caption +ToolWindow +AlwaysOnTop +LastFound -DPIScale
	if (MG_ExNaviTransBG) {
...
{
	global
	if (WinExist("ahk_id " MGW_hwHintsAdv)) {
		Gui, MGW_HintsAdv:Destroy
	}
	if (WinExist("ahk_id " MGW_hwHintsArw)) {
		Gui, MGW_HintsArw:Destroy
	}
}
...
MG_SetArrowHintsBgColor()
{
	...
	if (MG_ExNaviTransBG)
	{
		...
		Gui, MGW_HintsArw:+LastFound
		WinSet, TransColor, %colorBG%%trans%
	}
	Gui, MGW_HintsArw:Color, %colorBG%
}
...
MG_UpdateHintsArrows()
{
	...
	hMutex := MG_GetMutex(MG_MUTEX_HINTSARW, 0)
	if (!hMutex) {
		return
	}
	;...........................................................................
	...
	if ((szArrows!=MG_PrevGesture) || bDpiChanged)
	{
		...
		local hDC := DllCall("GetWindowDC", "Ptr",MGW_hwHintsArw, "Ptr")
		...
		if (MG_UseExNavi==1) {
			MG_UpdateHintsArrows1(hDC, szGesture, szArrows, winX, winY, bgc)
		} else if (MG_UseExNavi==2) {
			MG_UpdateHintsArrows2(hDC, szGesture, szArrows, winX, winY, bgc)
		}
		DllCall("SelectObject", "Ptr",hDC, "Ptr",hFntOld)
		DllCall("DeleteObject", "Ptr",hFont)
		DllCall("ReleaseDC", "Ptr",MGW_hwHintsArw, "Ptr",hDC)
	}
	;...........................................................................
	; Only Move Window
	else if (prevX!=winX || prevY!=winY)
	{
		Gui, MGW_HintsArw:Show, x%winX% y%winY% NA
	}
...
;-------------------------------------------------------------------------------
; Update Arrow Hints Type 1
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_UpdateHintsArrows1(hDC, szGesture, szArrows, winX, winY, clrBG)
{
	global
	...
	if (winW && winH)
	{
		Gui, MGW_HintsArw:Show, x%winX% y%winY% w%winW% h%winH% NA
...
;-------------------------------------------------------------------------------
; Update Arrow Hints Type 2
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_UpdateHintsArrows2(hDC, szGesture, szArrows, winX, winY, clrBG)
{
	global
	local size
	VarSetCapacity(size, 8, 0)
...
{
	local w, h, rc, hBrush
	WinGetPos,,, w, h, ahk_id %MGW_hwHintsArw%
	VarSetCapacity(rc, 16, 0)
	NumPut(w, rc,  8, "UInt")
	NumPut(h, rc, 12, "UInt")
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
MG_UpdateHintsAdvanced(iListOpe:=0)
{
	local hMutex, szGesture, winX, winY, newDPI, bDpiChanged, hDC, dwStyle
	static prevX:=0, prevY:=0

	if (iListOpe==0 && MG_UseExNavi!=3 && MG_UseExNavi!=4) {
		return
	}
	hMutex := MG_GetMutex(MG_MUTEX_HINTSADV, 0)
	if (!hMutex) {
		return
	}
	;...........................................................................
	; Check Gesture Conditions
	szGesture := MG_NaviPrst ? MG_NaviPrstStr : MG_Gesture
	if (!szGesture) {
		MG_ReleaseMutex(hMutex)
		return
	}
	...
	if ((szGesture!=MG_PrevGesture) || iListOpe || bDpiChanged) {
		hDC := DllCall("GetWindowDC", "Ptr",MGW_hwHintsAdv, "Ptr")
		...
		if (MG_UseExNavi==3 && iListOpe!=1) {
			MG_UpdateHintsAdvanced1(hDC, winX, winY, szGesture)
		} else {
			MG_UpdateHintsAdvanced2(hDC, winX, winY, szGesture)
		}
		DllCall("DeleteObject", "Ptr",MG_hFntBtn)
		DllCall("DeleteObject", "Ptr",MG_hFntDir)
		DllCall("DeleteObject", "Ptr",MG_hFntAct)
		DllCall("ReleaseDC", "Ptr",MGW_hwHintsAdv, "Ptr",hDC)
	}
	;...........................................................................
	; Only Move Window
	else if ((MG_AdNaviPosition==0) && (prevX!=winX || prevY!=winY)) {
		WinGet, dwStyle, Style, ahk_id %MGW_hwHintsAdv%
		if (dwStyle & 0x10000000) {
			Gui, MGW_HintsAdv:Show, x%winX% y%winY% NA
		}
	}
	prevX:=winX, prevY:=winY
...
MG_UpdateHintsAdvanced1(hDC, winX, winY, szGesture)
{
	local gesW, gesH, actW, actH, winW, winH
	...
	if (InStr(MG_ActionStr, MG_HIDEHINTS)) {
		MG_ActionStr := ""
	}
	actW := actH := 0
	if (MG_ActionStr) {
	...
	winW := gesW + actW + MG_AdjustToDPI(MG_AdNaviPaddingL) + MG_AdjustToDPI(MG_AdNaviPaddingR)
	winH := (gesH>actH ? gesH : actH) + MG_AdjustToDPI(MG_AdNaviPaddingT) + MG_AdjustToDPI(MG_AdNaviPaddingB)
	if (! winW || !winH) {
		return
	}
	MG_SetAdNaviPos(winX, winY, winW, winH)
	Gui, MGW_HintsAdv:Show, x%winX% y%winY% w%winW% h%winH% NA
...
MG_DrawTrail()
{
	...
	local hPen := DllCall("CreatePen", Int,0, Int,MG_AdjustToDPI(MG_TrailWidth), Int,MG_TrailColor2)
...
MG_EndOperation()
{
	...
	Loop, % MG_hMutex.MaxIndex() {
		MG_ReleaseMutexObject(MG_hMutex[A_Index])
	}
}
