...
MG_CheckLanguage()
if ((ME_CmdLineArg != "/ini2ahk") && MG_SearchPlugins() && MG_CheckConfigFiles()) {
	Reload
}
InitGlobals()
...
;
;-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
#If (WinActive("ahk_id " ME_hWndMain) && !IsControlActive("Config_Hotkey", true))
~^1::		SwitchTab(1)
~^2::		SwitchTab(2)
~^3::		SwitchTab(3)
~^4::		SwitchTab(4)
...
#If (WinActive("ahk_id " ME_hWndMain) && IsControlActive("Config_Hotkey", true))
Esc::		
	GuiControl, MEW_Main:, %MG_FocusCtrl%, Esc
	OnHotkeyChange(MG_FocusCtrl)
return
#If
...
InitConfOthers()
{
	...
	Config_HotkeyAbort		= Esc
	Config_HotkeyShowList	= F1
...
ShowLicenseDlg()
{
	...
	Gui, MEW_License:Add, Text, x+4 yp+2 vAhkWebsite gOnAhkLinkClick, AutoHotkey Official Website
	Gui, MEW_License:Add, Text, x+30 vAhkSource gOnAhkLinkClick, AutoHotkey Source Code
	...
	ME_hLinkDlg	:= ME_hLicenseDlg
	ME_LinkGUI	:= "MEW_License"
	ME_LinkFont	:= "S10"
	ME_LinkCtrl	:= [ "AhkWebsite", "AhkSource" ]
	ME_bLinkHover := [ 0, 0 ]
...
OnSetcursor(wParam, lParam, uMsg, hWnd)
{
	...
	if (WinExist("ahk_id " ME_hLinkDlg) && (hWnd = ME_hLinkDlg)) {
		CoordMode, Mouse, Client
		MouseGetPos, x, y
		CoordMode, Mouse, Screen
		hCursor := 0
		Loop, % ME_LinkCtrl.MaxIndex() {
			GuiControlGet, rc, %ME_LinkGUI%:Pos, % ME_LinkCtrl[A_Index]
			if (x>=rcX && x<(rcX+rcW) && y>=rcY && y<(rcY+rcH)) {
				hCursor := DllCall("LoadCursor", Ptr,0, UInt,32649, Ptr)
				if (!ME_bLinkHover[A_Index]) {
					ME_bLinkHover[A_Index] := 1
					Gui, %ME_LinkGUI%:Font, % ME_LinkFont " cRed Underline"
					GuiControl, %ME_LinkGUI%:Font, %  ME_LinkCtrl[A_Index]
				}
			} else {
				if (ME_bLinkHover[A_Index]) {
					ME_bLinkHover[A_Index] := 0
					Gui, %ME_LinkGUI%:Font, % ME_LinkFont " cBlue Underline"
					GuiControl, %ME_LinkGUI%:Font, %  ME_LinkCtrl[A_Index]
				}
			}
		}
...
CreateMainTab()
{
	local width, width2, height, top, bw, bl, pad, tblText
	...
	Gui, MEW_Main:Add, Button, xs+%bl% yp-4 w%bw% h25 vBAddAction	gBAddActionPress, %ME_LngButton005%
	SetButtonIcon("BAddAction", Icon_Add)
	Gui, MEW_Main:Add, ListView,xs ys+22 w%ME_GListW1% h%ME_GListH% Section Grid vLVGesture gLVGestureEvents -Multi NoSortHdr AltSubmit, % ME_LngListView002 "`n "
	width := ME_GListW1 - 22
	width2 := width * ME_GListR // 100
	LV_ModifyCol(1, width2)
	LV_ModifyCol(2, width - width2)
	LV_ModifyCol(3, 0)
	Gui, MEW_Main:Add, Button, x+1 w20 h20 +0x40 vBReleaseGesture gOnReleaseGesturePress Disabled
	SetButtonIcon("BReleaseGesture", Icon_Delete, ME_LngTooltip001)

	; Script editor
	top := ME_GListH+9
	Gui, MEW_Main:Add, Text, Section xs ys+%top% h16 vLabel3, %ME_LngText004%
	Gui, MEW_Main:Add, Button, xs+%bl% yp-6 w%bw% h25 vBUpdateAction	gBUpdateActionPress	Disabled, %ME_LngButton006%
	SetButtonIcon("BUpdateAction", Icon_Apply)
	EnblUpdateAction := "Disable"

	height := ME_ListH - ME_GListH - 89
	Gui, MEW_Main:Font, %ME_ScriptSize%, %ME_ScriptFont%
	Gui, MEW_Main:Add, Edit,   xs ys+22 w%ME_GListW1% h%height% Section vEAction	gOnActionEditModify -Wrap WantTab T12 +0x00100000 Disabled
	Gui, MEW_Main:Font
	Gui, MEW_Main:Add, Button, x+1 w20 h20 +0x40 vBEditAction gEditAction Disabled
	SetButtonIcon("BEditAction", Icon_Edit, ME_LngTooltip003)
	Gui, MEW_Main:Add, Button, xp y+4 w20 h20 +0x40 vBClearAction gOnClearActionPress	Disabled
...
CreateTargetsTab()
{
	...
	; Target list
	Gui, MEW_Main:Add, Button, Section w25 h25 +0x40 vBTargetNew gTargetNew
	SetButtonIcon("BTargetNew", Icon_Add, ME_LngTooltip011)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBTargetUp gTargetUp	Disabled
	SetButtonIcon("BTargetUp", Icon_Up, ME_LngTooltip012)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBTargetDown gTargetDown Disabled
	SetButtonIcon("BTargetDown", Icon_Down, ME_LngTooltip013)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBTargetSort gTargetSort
	SetButtonIcon("BTargetSort", Icon_Sort, ME_LngTooltip014)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBTargetDup gDuplicateTarget Disabled
	SetButtonIcon("BTargetDup", Icon_Dup, ME_LngTooltip015)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBTargetDelete gTargetDelete Disabled
	SetButtonIcon("BTargetDelete", Icon_Delete, ME_LngTooltip001)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBFoldTarget gSwitchTargetFolding
	...
	LV_ModifyCol(1, width2)
	LV_ModifyCol(2, width - width2)
	Gui, MEW_Main:Add, Button, x+1 yp+23 w20 h20 +0x40 vBRuleUp gRuleUp Disabled
	SetButtonIcon("BRuleUp", Icon_Up, ME_LngTooltip012)
	Gui, MEW_Main:Add, Button, y+20 w20 h20 +0x40 vBRuleDelete gRuleDelete Disabled
	SetButtonIcon("BRuleDelete", Icon_Delete, ME_LngTooltip001)
	Gui, MEW_Main:Add, Button, y+20 w20 h20 +0x40 vBRuleDown gRuleDown Disabled
	...
	Gui, MEW_Main:Add, Button, x+0	yp-1 w22 h22 +0x40 vBClearRule gClearRulePress Disabled
...
CreateGesturesTab()
{
	...
	; Gesture list
	Gui, MEW_Main:Add, Button, Section w25 h25 +0x40 vBGestureNew gGestureNew
	SetButtonIcon("BGestureNew", Icon_Add, ME_LngTooltip021)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBGestureUp gGestureUp   Disabled
	SetButtonIcon("BGestureUp", Icon_Up, ME_LngTooltip012)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBGestureDown gGestureDown Disabled
	SetButtonIcon("BGestureDown", Icon_Down, ME_LngTooltip013)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBGestureSort gGestureSort
	SetButtonIcon("BGestureSort", Icon_Sort, ME_LngTooltip014)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBGestureDup gDuplicateGesture Disabled
	SetButtonIcon("BGestureDup", Icon_Dup, ME_LngTooltip015)
	Gui, MEW_Main:Add, Button, x+0 w25 h25 +0x40 vBGestureDel gGestureDelete Disabled
	...
	Gui, MEW_Main:Add, Button, x+1 ys w20 h20 +0x40 vBGesturePatternUp gGesturePatternUp Disabled
	SetButtonIcon("BGesturePatternUp", Icon_Up, ME_LngTooltip012)
	Gui, MEW_Main:Add, Button, y+20 w20 h20 +0x40 vBGesturePatternDelete gGesturePatternDelete Disabled
	SetButtonIcon("BGesturePatternDelete", Icon_Delete, ME_LngTooltip001)
	Gui, MEW_Main:Add, Button, y+20 w20 h20 +0x40 vBGesturePatternDown gGesturePatternDown Disabled
	...
	Gui, MEW_Main:Add, Button, x+0	yp-1 w22 h22 +0x40 vBClearGesture gClearGesturePress Disabled
	...
	Gui, MEW_Main:Add, Button, xs y+5  w142 h46 vBButtonUp	 gOnBButtonUp	Disabled,			%ME_LngButton013%

	; Directions
	left := 152
	Gui, MEW_Main:Add, Text, xs+%left% ys-20 vLabel23 , %ME_LngText022%
	Gui, MEW_Main:Font, S14, Wingdings
	Gui, MEW_Main:Add, Button, xs+%left% ys		w32 h32 vBStrokeUL	gDir7 Disabled,			% Chr(0xEB)
	...
	Gui, MEW_Main:Add, Button, xs+%left% y+2	w32 h32 vBStrokeL	gDir4 Disabled,			% Chr(0xE7)
	Gui, MEW_Main:Add, Button, x+34				w32	h32 vBStrokeR	gDir6 Disabled,			% Chr(0xE8)
	Gui, MEW_Main:Add, Button, xs+%left% y+2	w32 h32 vBStrokeDL	gDir1 Disabled,			% Chr(0xED)
	...
	Gui, MEW_Main:Add, Button, y+5 w20 h20 +0x40 vBActionUp gActionUp Disabled
	SetButtonIcon("BActionUp", Icon_Up, ME_LngTooltip012)
	Gui, MEW_Main:Add, Button, y+20 w20 h20 +0x40	vBActionDelete gBActionDeletePress Disabled
	SetButtonIcon("BActionDelete", Icon_Delete, ME_LngTooltip001)
	Gui, MEW_Main:Add, Button, y+20 w20 h20 +0x40	vBActionDown gActionDown Disabled
...
CreateRecognitionTab()
{
	...
	width := width2 - width - 114
	Gui, MEW_Main:Add, CheckBox, +0x0400 x+30 ys+18 w%width% h50 vConfig_FirstLongOnly, %ME_LngCheckBox022%
...
CreateOthersTab()
{
	...
	Gui, MEW_Main:Tab, 7
	tblText := Array(ME_LngText451, ME_LngText452, ME_LngText453, ME_LngText454, ME_LngText455)
	width := GetMaxTextLength(tblText)+8
	...
	Gui, MEW_Main:Add, GroupBox, xs y+20 w%wAvlbl% h165,%ME_LngGroupBox012%
	...
	Gui, MEW_Main:Add, Hotkey,x+0 w%width2% vConfig_HotkeyAbort gOnHotkeyChange

	Gui, MEW_Main:Add, Text,xs+12 y+8 w%width% vLabel134,%ME_LngText454%
	Gui, MEW_Main:Add, Hotkey,x+0 w%width2% vConfig_HotkeyShowList gOnHotkeyChange

	Gui, MEW_Main:Add, Text,xs+12 y+8 w%width% vLabel135,%ME_LngText455%
	Gui, MEW_Main:Add, Hotkey,x+0 w%width2% vConfig_HotkeyNavi gOnHotkeyChange


	Gui, MEW_Main:Add, GroupBox,xs y+22 w%wAvlbl% h62, %ME_LngGroupBox015%
	Gui, MEW_Main:Add, Button, xs+16 yp+20 w300 h30 gDlgRegActvtExclud, %ME_LngButton022%


	Gui, MEW_Main:Add, GroupBox,xs y+22 w%wAvlbl% h52, %ME_LngGroupBox013%
	...
	Gui, MEW_Main:Add, Text,xs+40 y+6 vLabel136, %ME_LngText456%
...
AdjustTextPos()
{
	...
	Loop, 140
	{
		...
		if (!ErrorLevel)
		{
			posY+=MG_AdjustToDPI(4, -1)
			posX+=MG_AdjustToDPI(2, -1)
...
{
	local hCtrl, hTip, hInst, ti, size, ofs
	static objTips := []

	hCtrl := ControlGetHandle(szCtrl, szWnd)
	size := 4*3 + 16 + A_PtrSize*4
	VarSetCapacity(ti, size, 0),	ofs := 0
	NumPut(size, ti, ofs, "UInt"),	ofs += 4
	NumPut(0x11, ti, ofs, "UInt"),	ofs += 4
	NumPut(hCtrl, ti, ofs,  "Ptr"),	ofs += A_PtrSize
	NumPut(hCtrl, ti, ofs,  "Ptr"),	ofs += A_PtrSize*2+16
	...
	if (hTip) {
		...
		hTip := DllCall("CreateWindowExW", UInt,8, Str,"tooltips_class32", Str,"", UInt,0x80000003
							,Int,0, Int,0, Int,10, Int,10, Ptr,hCtrl, Ptr,0, Ptr,hInst, Ptr,0, Ptr)
...
IsAnyListActive()
{
	GuiControlGet, focused, MEW_Main:FocusV
	return (focused="TVTarget1" || focused="TVTarget2" || focused="LVRule"
		 || focused="LVGesture" || focused="LBGesture" || focused="LVAction"
		 || focused="LBGesturePattern")
}

;-------------------------------------------------------------------------------
; Check whether the specified control is activated
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
IsControlActive(target, bPrefix:=false)
{
	global
	GuiControlGet, MG_FocusCtrl, MEW_Main:FocusV
	if (bPrefix) {
		return (InStr(MG_FocusCtrl, target)==1)
	} else {
		return (MG_FocusCtrl = target)
	}
}
...
{
	local szTemp, lenS, lenE, posS, posE, szURL, szS, szE, szLink, Ew, Bx, Bw, Bs, bOK
	...
	szTemp := RegExReplace(szPrompt, "U)###.+###")
	Gui, MEW_InBox:Add, Text, vIbText, %szTemp%
	GuiControlGet, rcCtrl, MEW_InBox:Pos, IbText
	Ew := (rcCtrlW >= 300) ? rcCtrlW : 300
	szURL := ""
	posS := InStr(szDefault, "#LinkID#")
	if (posS) {
		lenS := StrLen("#LinkID#")
		szURL := SubStr(szDefault, posS+lenS)
	}
	posS := InStr(szPrompt, "###LinkStart###")
	posE := InStr(szPrompt, "###LinkEnd###")
	if (szURL && posS && posE) {
		lenS := StrLen("###LinkStart###")
		lenE := StrLen("###LinkEnd###")
		szS := SubStr(szPrompt, 1, posS-1)
		szLink := SubStr(szPrompt, posS+lenS, posE-posS-lenS)
		szE := SubStr(szPrompt, posE+lenE)
		WinHide, % "ahk_id " ControlGetHandle("IbText", "MEW_InBox")
		Gui, MEW_InBox:Add, Text, yp+12 vTextS, %szS%
		Gui, MEW_InBox:Font, S11 cBlue Underline
		Gui, MEW_InBox:Add, Text, x+1 yp vTipsLink gOnTipsLinkClick, %szLink%
		Gui, MEW_InBox:Font
		Gui, MEW_InBox:Font, S11
		Gui, MEW_InBox:Add, Text, x+1 yp vTextE, %szE%
		szURL := MG_WEBSITEURL (MG_Language="Japanese" ? "" : "en/") MG_TIPSPAGE "#" szURL
	}
	if (!InStr(szDefault, "#NoInput#")) {
	...
	ME_hLinkDlg	:= ME_hWndInBox
	ME_LinkGUI	:= "MEW_InBox"
	ME_LinkFont	:= "S11"
	ME_LinkCtrl	:= [ "TipsLink" ]
	ME_bLinkHover := [ 0 ]
	OnMessage(0x0020, "OnSetcursor")
...
AdjustDialogHeight(bAdjPos=false)
{
	...
	static tblY := [ "Label3", "BUpdateAction", "EAction"
				   ...
				   , "BGesturePatternBS", "BFromClipboard", "BHelp", "BSaveExit", "BExit" ]
...
OnReleaseGesturePress() {
	global
	ReleaseGesture(Gesture_Editing, Action_Editing)
	GuiControl, MEW_Main:Focus, LVGesture
}
...
EditAction()
{
	...
	GuiControlGet, ctrlFocus, MEW_Main:FocusV
	...
	GuiControl, MEW_Main:Focus, % ctrlFocus="LVGesture" ? "LVGesture" : "EAction"
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
OnHotkeyChange(target_ctrl:="")
{
	local newkey, cmpkey
		, tbl := [ "Config_HotkeyReload", "Config_HotkeyEnable", "Config_HotkeyNavi"
				 , "Config_HotkeyShowList", "Config_HotkeyAbort" ]

	Gui, MEW_Main:Submit, Nohide
	if (A_GuiControl) {
		target_ctrl := A_GuiControl
	}
	newkey := %target_ctrl%
	Loop, % tbl.MaxIndex() {
		if (tbl[A_Index] != target_ctrl) {
			cmpkey := tbl[A_Index]
			cmpkey := %cmpkey%
			if (newkey == cmpkey) {
				GuiControl, MEW_Main:, % tbl[A_Index]
				break
			}
		}
	}
...
DlgTaskList(ByRef title, ByRef class, ByRef exe)
{
	...
	{
		hWnd := nWnd%A_Index%
		if (hWnd==ME_hWndActvEx || hWnd==ME_hWndTskLst || !MG_IsActivationTarget(hWnd, true, false)) {
			continue
		}
...
OnAdjustDlgHeight()
{
	...
	GuiControl, MEW_Main:%MG_CtrlStat%, Label136
...
;-------------------------------------------------------------------------------
GetRectRelative() {
	GetRectByDrag(rx, ry, rw, rh, tx, ty, tw, th, 0)
	DlgRectSettings(rx, ry, rw, rh, tx, ty, tw, th, 0)
}
GetRectAbsolute() {
	GetRectByDrag(rx, ry, rw, rh, tx, ty, tw, th, 1)
	DlgRectSettings(rx, ry, rw, rh, tx, ty, tw, th, 1)
}

;-------------------------------------------------------------------------------
; Get Rectangle Coordinates by Mouse Dragging
;	rcX, rcY, rcW, rcH	: Rectangular Region Selected by User
;	tgX, tgY, tgW, tgH	: Rectangular Region of Target Window
;	target				: 0:Target is Active Window
;						  1:Target is Screen
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
GetRectByDrag(ByRef rcX, ByRef rcY, ByRef rcW, ByRef rcH
			, ByRef tgX, ByRef tgY, ByRef tgW, ByRef tgH, target) 
{
	...
	if (target==0) {
		MouseGetPos, , , hwnd
		WinGetPos, tgX, tgY, tgW, tgH, ahk_id %hwnd%
	...
	While (GetKeyState("LButton", "P")) {
		...
		winX := (x1 < x2) ? x1 : x2
		winY := (y1 < y2) ? y1 : y2
		winW := MG_AdjustToDPI(abs(x1 - x2), -1)
		winH := MG_AdjustToDPI(abs(y1 - y2), -1)
...
;														Implemented by Pyonkichi
;-------------------------------------------------------------------------------
DlgRectSettings(rcX, rcY, rcW, rcH, tgX, tgY, tgW, tgH, target)
{
...
;				LV_Add("", Gesture_%ges%_Name, MakeActionSummaryStr(Gesture_%ges%_%A_Index%_Action), ges)
				act := MakeActionSummaryStr(Gesture_%ges%_%A_Index%_Action)
				act := RegExReplace(act, "i)" MG_HIDEHINTS)
				LV_Add("", Gesture_%ges%_Name, act, ges)
				break
			}
		}
	}
...
;-------------------------------------------------------------------------------
; On Add Gesture to Slected Target Press
;														Implemented by lukewarm
;														Modified by Pyonkichi
;-------------------------------------------------------------------------------
BAddActionPress() {
...
InitActionTemplates()
{
	...
	MG_AddActionTemplate("SameClass", ActionName043, "ActRestoreSameClass")
	MG_AddActionTemplate("SameClass", ActionName044, "ActTileSameClass")
	...
	MG_AddActionTemplate("Script", ActionName092, "MG_SaveGesture()%[" ME_LngMessage120 "%<#NoInput# #LinkID#save_gesture1>%]%")
	MG_AddActionTemplate("Script", ActionName093, "MG_SetBypass()%[" ME_LngMessage120 "%<#NoInput# #LinkID#save_gesture2>%]%")
	MG_AddActionTemplate("Script", ActionName094, "MG_SetActiveAsTarget()")
...
ActMoveWindow()
{
	...
	Gui, MEW_WMS:Add, CheckBox, x26 y+14 h14 vWmsAdj Checked, %ME_LngCheckBox113%
...
ActMinimizeSameClass() {
	global
	DlgOperateSameClass("Minimize", ME_LngCapt026)
}
ActCloseSameClass() {
	global
	DlgOperateSameClass("Close", ME_LngCapt027)
}
ActRestoreSameClass() {
	global
	DlgOperateSameClass("Restore", ME_LngCapt028)
}
DlgOperateSameClass(szOpe, szCaption)
{
	local y, width, Bx, Bw, Bs, tblText, szInc, szExc
	Gui, MEW_OSC:-MaximizeBox -MinimizeBox +HwndME_hWndOSC +OwnerMEW_Main +Delimiter`n +LastFound
	if (szOpe = "Restore") {
		y := 15
		Gui, MEW_OSC:Add, CheckBox, % "x20 y" y " vChkRestorePrevMinWnd Checked", %ME_LngCheckBox114%
		y += 30
	} else {
		y:=12
	}
	Gui, MEW_OSC:Add, Text, % "x20 y" y " Section", %ME_LngText571%

	tblText := Array(ME_LngText572, ME_LngText573)
	width := GetMaxTextLength(tblText)+8
	...
	Gui, MEW_OSC:Show, AutoSize, %szCaption%
...
OnAcceptOSC:
	Gui, MEW_OSC:Submit
	if (szOpe="Restore" && ChkRestorePrevMinWnd) {
		szOpe := "RestoreMinWnd"
	}
	szInc := szExc := ""
	if (EIncludeTitle || EExcludeTitle) {
		szInc := EIncludeTitle ? ", """ EIncludeTitle """" : ","
		szExc := EExcludeTitle ? ", """ EExcludeTitle """" : ""
	}
	MG_AddActionScript("MG_OperateSameClass(""" szOpe """" szInc szExc ")")
	Gui, MEW_OSC:Destroy
	return
...
ActTileSameClass()
{
	...
	Gui, MEW_TSC:Show, AutoSize, %ME_LngCapt029%
...
ToAhk()
{
	...
	if (Config_HotkeyAbort || Config_HotkeyShowList) {
		szBuf .= "`n#if MG_Active`n"
		if (Config_HotkeyAbort) {
			szBuf .= Config_HotkeyAbort "::MG_Abort()`n"
		}
		if (Config_HotkeyShowList) {
			szBuf .= Config_HotkeyShowList "::MG_ShowGestureList()`n"
		}
		szBuf .= "#if`n"
	}
